...
def SystemD3(Stress, LamValue):
    ...
    # Parameters for stress input
    T=3600*6;                   A=Stress;                      lemda=LamValue
...
###############
# Data ploting
Data1=SystemD1(3.25,700.0); Data2=SystemD1(3.8, 700.0)
Data3=SystemD1(3.25, 500.0); Data4=SystemD1(3.8, 500.0)
...
# Data ploting
Data11=SystemD2(3.25,300.0); Data21=SystemD2(3.8, 300.0)
Data31=SystemD2(3.25, 1.0); Data41=SystemD2(3.8, 1.0)
...
###############
# Data ploting
Data12=SystemD3(3.0,0.07/3600); Data22=SystemD3(6.0,0.07/3600)
Data32=SystemD3(3.0,0.03/3600); Data42=SystemD3(6.0,0.03/3600)
...
##################
import seaborn as sns; sns.set()
Xdat=np.linspace(3.25,3.8,200)
Ydat=np.linspace(500.0,700,200)[::-1]
Index=[0,66,132,199]
Xticks=[]; Yticks=[]
for i in range(len(Xdat)):
    if i in Index: Xticks.append(round(Xdat[i],2)); Yticks.append(round(Ydat[i],0))
    else: Xticks.append(''); Yticks.append('')

Data=np.genfromtxt('./data/RecTiD.dat')
plt.subplot(grid[0:200, 110:260])
ax=sns.heatmap(Data[:][:],linewidth=0.0,yticklabels=Yticks,xticklabels=Xticks,rasterized=True, cmap='RdYlGn_r')
cbar = ax.collections[0].colorbar
cbar.set_label(r'T$_{ps}$ (hrs)', labelpad=5, fontsize=fs1)
ax.set_xticks(list(np.linspace(5,200,200)))
...
plt.xlabel(r'Magnitude of stress ($I$)', fontsize=fs);
plt.ylabel(r'$K_{3}$', fontsize=fs)

Xdat1=np.linspace(3.25,3.8,200)
Ydat1=np.linspace(1.0,300,200)[::-1]
Index1=[0,66,132,199]
Xticks1=[]; Yticks1=[]
for i in range(len(Xdat1)):
    if i in Index1: Xticks1.append(round(Xdat1[i],2)); Yticks1.append(round(Ydat1[i],0))
    else: Xticks1.append(''); Yticks1.append('')

Data1=np.genfromtxt('./data/RecTiKgD.dat')
plt.subplot(grid[240:440, 110:260])
ax=sns.heatmap(Data1[:][:],linewidth=0.0,yticklabels=Yticks1,xticklabels=Xticks1,rasterized=True, cmap='RdYlGn_r')
cbar = ax.collections[0].colorbar
cbar.set_label(r'T$_{ps}$ (hrs)', labelpad=5, fontsize=fs1)
ax.set_xticks(list(np.linspace(5,200,200)))
plt.xticks(fontsize=fs-3); plt.yticks(fontsize=fs-3); plt.gcf().axes[-1].tick_params(labelsize=15)
...
plt.xlabel(r'Magnitude of stress ($I$)', fontsize=fs);
plt.ylabel(r'$K_{4}$', fontsize=fs)

Xdat2=np.linspace(3.0,6.0,200)
Ydat2=np.linspace(0.03,0.07,200)[::-1]
Index2=[0,66,132,199]
Xticks2=[]; Yticks2=[]
...
Data2=np.genfromtxt('./data/RecTiLD.dat')
plt.subplot(grid[480:680, 110:260])
ax=sns.heatmap(Data2[:][:],linewidth=0.0,yticklabels=Yticks2,xticklabels=Xticks2,rasterized=True, cmap='RdYlGn_r')
cbar = ax.collections[0].colorbar
cbar.set_label(r'T$_{ps}$ (hrs)', labelpad=5, fontsize=fs1)
ax.set_xticks(list(np.linspace(5,200,200)))
plt.xticks(fontsize=fs-3); plt.yticks(fontsize=fs-3); plt.gcf().axes[-1].tick_params(labelsize=15)
plt.title('C', fontsize=fs, loc='left')
plt.xlabel('Magnitude of stress', fontsize=fs); plt.ylabel(r'$\lambda$', fontsize=fs)
